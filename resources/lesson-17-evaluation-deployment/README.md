...
### 问题 1：为什么 p95 对输入法候选栏比 tokens/s 更重要？

**参考答案：** 用户感知的是一次按键何时出现完整候选栏，而不是 GPU 每秒能处理多少 Token。tokens/s 可以很高，但某些请求若因长分支、refill 或资源竞争出现高尾延迟，用户仍会感到卡顿。p95 更能反映“较差但常见”的等待体验。

### 问题 2：如何设计 Benchmark 避免把首次 JIT 编译算进每次请求？

**参考答案：** 先在同一模型、同一配置下执行固定数量的 warmup，让 Kernel 编译、缓存初始化和 allocator 预热完成，再开始计时正式样本；同时明确模型加载和一次性初始化是否在计时边界之外。所有对照 Runtime 必须使用同样规则。

### 问题 3：为什么固定候选排序与开放生成必须拆 Lane？

**参考答案：** 固定候选排序控制了候选集合，只检验模型对既定中文候选的条件概率排序；开放生成还包含采样、停止、候选覆盖、过滤和多样性。一个 Runtime 可以固定排序完全持平，但开放候选池仍变差，因此两者不能互相替代。

### 问题 4：如果显存多 10 MiB 但 p95 减半，怎样判断值不值？

**参考答案：** 要看目标设备的显存余量和产品目标。如果多 10 MiB 不造成 OOM、不挤压其他关键模块，而 p95 显著下降，就可能是很好的 Pareto 改善；若目标设备只剩几 MiB 余量，则同一取舍不可接受。性能结论必须绑定设备约束。

### 问题 5：为什么“测试全部通过”与“模型生产可用”是两层结论？

**参考答案：** 单元/GPU 测试证明的是实现不变量，例如 Page 回收、排序路径、取消和导出结构正确；生产可用还要求真实用户语义质量、设备覆盖、长期稳定、故障恢复和人工接受度。代码正确是必要条件，不是最终产品质量证明。
