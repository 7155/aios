...
### 检验问题：为什么不仍然把 8 条候选当成 8 个普通 request，交给通用 Scheduler？

**参考答案：** 因为这 8 条分支不是 8 个彼此独立的请求。它们共享同一个 Prefix、同一生命周期、同一个 latest-wins 取消边界，还应该物理共享 Prefix KV；最终也不是各自返回，而是共同参与过滤、去重、MMR 和 refill 决策。普通 Scheduler 只知道“请求 A/B/C”，看不到“这 8 条属于同一个 CandidateGroup”的组级语义，因此会丢失最重要的共享与治理机会。
...
### 问题 1：为什么输入法不应该为了利用 Continuous Batching，主动等待其他请求？

**参考答案：** 当前产品是本地单用户，通常没有第二个独立用户请求可以等待。人为等待只会增加按键到候选栏的延迟，而不会创造有价值的并发。真正存在的并行性是同一个 Prefix 内的候选分支，因此应该优化 CandidateGroup，而不是为了套用通用 serving 模型而制造队列。

### 问题 2：为什么“最终显示 3 条”不等于“只生成 3 条”？

**参考答案：** 原始生成还要经过过滤、显示去重、长度限制和多样性选择。三路中只要有一路非法或两路显示等价，候选栏就不足三条。因此首轮需要更大的探索池，再从中选出最终三条；当前实现是先 8 路，不足时再补 4 路。

### 问题 3：为什么 latest-wins 属于正确性，而不仅是性能优化？

**参考答案：** 用户继续输入后，旧 Prefix 已经不是当前输入状态。旧候选即使模型计算完全正确，也对应过时输入，如果回显就会造成 UI 闪回和错误建议。因此系统必须保证只有最新 generation 有交付资格；尽快取消旧计算只是进一步减少浪费。

### 问题 4：为什么不能拿“首步 8ms”宣称“Top-3 延迟 8ms”？

**参考答案：** 首步只覆盖 Prefix Prefill 和首 token 采样；用户真正等待的是多步 Decode、文本解码、过滤、去重、MMR，甚至 refill 完成后的三条候选。两个指标的计时边界不同，必须使用完整 Top-3 墙钟延迟评价候选栏体验。
